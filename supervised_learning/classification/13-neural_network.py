...
class NeuralNetwork:
    ...
    Public methods:
        def forward_prop(self, X): Calculates the forward propagation
        of the neural network
        def cost(self, Y, A): Calculates the cost using logistic regression
        def evaluate(self, X, Y): Evaluates the predictions
        of the neural network
        def gradient_descent(self, X, Y, A1, A2, alpha=0.05): Performs one
    ...
    def gradient_descent(self, X, Y, A1, A2, alpha=0.05):
        ...
        # Compute the gradients for W2, b2
        dz2 = A2 - Y
        dW2 = (1 / m) * np.matmul(dz2, A1.T)
        db2 = (1 / m) * np.sum(dz2, axis=1, keepdims=True)

        # Compute the gradients for W1, b1
